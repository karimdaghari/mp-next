...
function TreeItem({ data, depth }: TreeItemProps) {
  ...
  return (
    <div
      ...
      <div className='w-fit bg-white px-4 py-2 rounded-lg border'>
        <TypographyMuted className='text-[0.7rem]'>Catégorie</TypographyMuted>
      ...
      {data.agendas.map((agenda) => {
        ...
        return (
          <div
            key={agenda.id}
            style={{
              paddingLeft: `${(depth === 0 ? 1 : depth) * 0.5}rem `
            }}>
            <Card className='bg-white p-0 rounded-lg border'>
